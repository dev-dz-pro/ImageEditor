...
class Stresh1(QtWidgets.QLabel):
    ...
    def load_image(self):
        ...
        # TODO TO CHECK THE SCALLING EFFECTION TO THE REMAIN CODE and check other todo in the code
        self.pmap = QtGui.QPixmap(self.fname[0])
        self.is_scaled = False
        self.scalling_rate = None
        # if pmap.width() > pmap.height():
        #     if pmap.width() > 877:
        #         pmap = pmap.scaledToWidth(877)
        #         self.is_scaled = True
        # elif pmap.height() > pmap.width():
        #     if pmap.height() > 877:
        #         pmap = pmap.scaledToHeight(877)
        #         self.is_scaled = True


        # self.setAlignment(QtCore.Qt.AlignCenter)
        if self.pmap.width() > 620 and self.pmap.height() > 877:
            sh0 = self.pmap.height()/877
            sw0 = self.pmap.width()/620

            self.pmap = self.pmap.scaled(620, 877, QtCore.Qt.KeepAspectRatio)  #, QtCore.Qt.SmoothTransformation

            sh = self.pmap.height()/877
            sw = self.pmap.width()/620
            if sw == 1.0:
                self.scalling_rate = sw0
            if sh == 1.0:
                self.scalling_rate = sh0
...
class Ui_MainWindow(object):  
    def setupUi(self, MainWindow):
        ...
        self.label_1.setMaximumSize(QtCore.QSize(877, 877))
    ...
    def correct_input_angle(self):
        ...
        self.label_1.pmap = QtGui.QPixmap(outfile)
        self.label_1.is_scaled = False
        if self.label_1.pmap.width() > 620 and self.label_1.pmap.height() > 877:
            self.label_1.pmap = self.label_1.pmap.scaled(620, 877, QtCore.Qt.KeepAspectRatio)   # , QtCore.Qt.SmoothTransformation
            self.label_1.is_scaled = True
        self.label_1.setPixmap(self.label_1.pmap)


    def on_rotate_image(self, rotate_left=False):
    ...
    def on_expand_image(self, pad=0.0): 
        ...
        try:
            if not self.label_2.is_expanded:
                # self.label_2._image.save('tmp.png')
                # im = Image.open('tmp.png')
                pm = self.label_2._image.copy()
                ...
                if width > 620:
                    rate = 620 / width
                    expand_rate *= rate
                    # width, height = int(width * rate), int(height * rate)
                    # im = im.resize((width, height), Image.ANTIALIAS)
                if height > 877:
                    rate = 877 / height 
                    expand_rate *= rate
                    # width, height = int(width * rate), int(height * rate)
                    # im = im.resize((width, height), Image.ANTIALIAS) # Image.ANTIALIAS, 
                if width < 620 and height < 877:
                    if width > height:
                        rate = 620 / width
                        expand_rate *= rate
                        # width, height = 620, int(height * rate)
                        # im = im.resize((width, height), Image.ANTIALIAS)
                    else:
                        rate = 877 / height
                        expand_rate *= rate
                        # width, height = int(width * rate), 877
                        # im = im.resize((width, height), Image.ANTIALIAS)
                # im.save("scalled_3.png", "PNG")
                self.label_2.is_expanded = True
    ...
    def on_save_image(self, pad):
        ...
        if r > 1:
            # if not self.label_1.is_scaled:
            cv2.fillPoly(mask, np.int64([self.label_2.chosen_points ]), (255)) # cv2.fillPoly(mask, np.int64([self.label_2.chosen_points * r ]), (255))
            # else:
